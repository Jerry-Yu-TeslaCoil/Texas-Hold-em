 ...
 * @version 1.1
 */
public class ClassicTable implements CardTable {
     ...
     */
    public ClassicTable(TableConfig tableConfig) {
    ...
    @Override
    public synchronized void setTableConfig(TableConfig tableConfig) {
        this.tableConfig = tableConfig;
        this.playerFactory.setConfig(tableConfig);
        this.players.setMaxPlayers(tableConfig.maxPlayers());
    }
    ...
    public List<RecordReader> startRounds() {
        ...
        PlayerList players;
        synchronized (lock) {
            players = new PlayerCoil(this.players);
        }
        gameStateContext.setPlayers(players);
        ...
        for(CardPlayer cardPlayer : players.getPlayers()) {
